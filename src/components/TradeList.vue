<template>
  <div class="trades" :class="{ '-compact': compactRows, '-logos': this.showLogos }">
    <ul v-if="trades.length">
      <li
        ...
        :style="{ 'background-image': trade.image, 'background-color': trade.background, 'color': trade.foreground }"
      >
        ...
        <template v-else>
          ...
          <div class="trades__item__price" v-bind:slippage="trade.slippage">
    ...
    <div v-else class="trades__item -empty">Nothing to show, yet.</div>
  </div>
</template>
  ...
  methods: {
    ...
    onTrades(trades, silent = false) {
      for (let trade of trades) {
        ...
        if (trade[5] === 1) {
          ...
          if (size >= options.threshold * (trade[0] === 'bitmex' ? 1 : 0.75)) {
            this.appendRow(
              ...
              }</strong> <small>@ ${app.getAttribute(
                'data-symbol'
              )}${formatPrice(trade[2])}</small>`
            )
          }
          continue
        }
        ...
        if (
          options.useAudio &&
          ((options.audioIncludeAll && size > options.threshold * 0.1) ||
            size > options.significantTradeThreshold)
        ) {
          this.sfx &&
            !silent &&
        ...
        if (!options.threshold || size >= options.threshold) {
          this.appendRow(trade)
        }

        if (options.showSlippage) {
          slipRef[trade[0]] = trade[2]
        }
      }
    ...
    appendRow(trade, classname = [], message = null) {
      ...
      if (amount >= options.hugeTradeThreshold) {
        if (
          amount < options.rareTradeThreshold &&
          this.gifs.huge &&
          this.gifs.huge.length
        ) {
          image = this.gifs.huge[
            Math.floor(Math.random() * (this.gifs.huge.length - 1))
          ]
        }
      ...
      if (amount >= options.rareTradeThreshold) {
        if (this.gifs.rare && this.gifs.rare.length) {
          image = this.gifs.rare[
            Math.floor(Math.random() * (this.gifs.rare.length - 1))
          ]
        }
    ...
    getGifs(refresh) {
      ;[
        ...
        const storage = localStorage
          ? JSON.parse(localStorage.getItem(animation.threshold + '_gifs'))
          : null

        if (
          !refresh &&
          storage &&
          +new Date() - storage.timestamp < 1000 * 60 * 60 * 24
        ) {
    ...
    getTradeColor(trade) {
      ...
      const opacity = +(
        0.33 +
        Math.min(0.66, (amount / options.significantTradeThreshold) * 0.66)
      ).toFixed(2)
      ...
      return {
        background:
          'rgba(' + [color.r, color.g, color.b, opacity].join(',') + ')',
        foreground:
          'rgba(' +
          (luminance > 200 ? '0,0,0' : '255,255,255') +
          ',' +
          Math.min(1, opacity * 1.25) +
          ')'
      }
    }
  }
